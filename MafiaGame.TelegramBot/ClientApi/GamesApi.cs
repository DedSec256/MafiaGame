...
namespace TelegramBot.ClientApi
{
    class GamesApi : BaseApi
    {
        ...
        public async Task<GameRoomJson> AddPlayerInGame(long id)
        {
            var response = await client.GetAsync($"{client.BaseAddress}/games/{id}/players/add");
            if (!response.IsSuccessStatusCode)
                throw new HttpRequestException(response.StatusCode.ToString() + "\n" + response.ReasonPhrase);
            return await response.Content.ReadAsAsync<GameRoomJson>();
        }
        ...
        {
            var response = await client.GetAsync($"{client.BaseAddress}/games/{id}/players/{playerId}/delete");
            if (!response.IsSuccessStatusCode)
                throw new HttpRequestException(response.StatusCode.ToString() + "\n" + response.ReasonPhrase);
            return await response.Content.ReadAsAsync<GameRoomJson>();
        }
